...
if utils.getEnvironmentValueByKey('TRAIN_GENDER_CLASSFICATION_DATA') == 'TRUE':
    traindata.trainGenderClassficationData(
        'images/male/*.*', 1, 'images/female/*.*', 2, 50, 50)
...
while(True):
    ...
    faces = face_cascade.detectMultiScale(
        gray, scaleFactor=1.30, minNeighbors=5, minSize=(20, 20))
    ...
    if faceTime != 0 and (datetime.now().minute > prevMinute or (datetime.now().minute == 0 and prevMinute == 59)):
        analytics.recordStatistics(faceTime, round(
            runningFaceTime.total_seconds()), totalMaleFaces, totalFemaleFaces)
        prevMinute = datetime.now().minute
    ...
    for (x, y, w, h) in faces:
        ...
        cv2.putText(img, gender_classifier.get(predictedLabel),
                    (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0))
